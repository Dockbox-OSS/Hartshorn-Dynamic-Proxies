...
import org.dockbox.hartshorn.element.MethodInvoker;
...
import org.dockbox.hartshorn.loaders.ParameterLoader;
...
public class JavassistProxyMethodHandler<T> implements MethodHandler {
    ...
    public JavassistProxyMethodHandler(final ProxyManager<T> manager, final ApplicationProxier applicationContext) {
        ...
        try {
            ...
            if (this.manager.delegate().present()) return this.invokeDelegate(this.manager.delegate().get(), target, args);
                ...
                throw new IllegalStateException("Could not invoke local method " + localMethodContext.qualifiedName()
                        ...
                        + targetType.qualifiedName() + " of qualified type " + selfContext.qualifiedName() + "(isProxy=" + this.applicationContext().isProxy(self) + ")"
